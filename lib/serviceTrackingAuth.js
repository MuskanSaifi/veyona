...
/** True when request has adminToken cookie (same gate as middleware). */
export function isAdminRequest(req) {
  const verified = verifyTokenFromCookie(req, "adminToken", "admin");
  if (!verified.error) return true;
  // Middleware only checks cookie presence — keep the same for admin panel fetches
  return Boolean(req.cookies.get("adminToken")?.value);
}
