...
import { splitGstInclusive, CGST_RATE, SGST_RATE, IGST_RATE, COMBINED_GST_RATE } from "@/lib/invoiceGst";
...
export async function buildInvoicePdfBuffer(apt, billTo) {
  ...
  const customerState = apt.customer?.state || billTo?.state || "";
  const { taxable, cgst, sgst, igst, isInterState } = splitGstInclusive(total, customerState);
  ...
  const subRowH = 20;
  const rowH = 22;
  // Table font size. Named `tFs` (NOT `fs`) on purpose: a local `const fs`
  // would shadow the imported `fs` module across the whole function due to
  // the JS Temporal Dead Zone, breaking `fs.readFile(...)` in loadLogoImage.
  const tFs = 7.5;
  ...
  for (let i = 0; i < emptyRows; i++) {
    ...
    for (const col of cols) {
      ...
      if (row) {
        if (col.key === "sr") drawText(String(row.sr), cx + 4, rowBottom + 7, tFs);
        else if (col.key === "description") {
          wrapText(row.description, font, tFs, col.w - 8)
            .slice(0, 2)
            .forEach((ln, li) => drawText(ln, cx + 4, rowBottom + rowH - 12 - li * 10, tFs));
        } else if (col.key === "hsn") drawText(row.hsn, cx + 4, rowBottom + 7, tFs);
        else if (col.key === "gstPct") drawText(row.gstPct, cx + 4, rowBottom + 7, tFs);
        else if (col.key === "qty") {
          totalQty += row.qty;
          drawText(String(row.qty), cx + 4, rowBottom + 7, tFs);
        } else if (col.key === "rate") {
          const v = fmtRs(row.rate);
          drawText(v, cx + col.w - font.widthOfTextAtSize(v, tFs) - 4, rowBottom + 7, tFs);
  ...
  drawText(String(totalQty), tableX + cols[0].w + cols[1].w + cols[2].w + cols[3].w + 8, rowBottom + 6, tFs, true);
  const subAmt = fmtRs(linesSum - lineDiscount);
  drawText(subAmt, tableX + tableW - font.widthOfTextAtSize(subAmt, tFs) - 6, rowBottom + 6, tFs, true);
  ...
  const tCols = isInterState ? [40, 64, 64, 58] : [36, 50, 46, 46, 46];
  const tHead = isInterState
    ? ["GST %", "Taxable Amt", "IGST Amt", "Tax Amt"]
    : ["GST %", "Taxable Amt", "SGST Amt", "CGST Amt", "Tax Amt"];
  ...
  tx = taxX;
  const taxValues = isInterState
    ? [GST_PERCENT_LABEL, fmtRs(taxable), fmtRs(igst), fmtRs(igst)]
    : [GST_PERCENT_LABEL, fmtRs(taxable), fmtRs(sgst), fmtRs(cgst), fmtRs(cgst + sgst)];
  ...
  if (isInterState) {
    sumRow("Add : IGST (18%)", fmtRs(igst));
  } else {
    sumRow("Add : CGST (9%)", fmtRs(cgst));
    sumRow("Add : SGST (9%)", fmtRs(sgst));
  }
