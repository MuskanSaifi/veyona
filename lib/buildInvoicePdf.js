...
/**
 * @param {object} apt — lean appointment with customer, salon, service populated as in invoice route
 * @param {{ name?: string; phone?: string; email?: string }} billTo — displayed on invoice (customer + fallbacks)
...
export async function buildInvoicePdfBuffer(apt, billTo) {
  ...
  y -= 16;
  drawText(esc(apt.customer?.name || billTo?.name || "Customer"), 42, y, 12, true);
  y -= 14;
  drawText(esc(apt.customer?.phone || billTo?.phone || ""), 42, y, 10);
  y -= 13;
  drawText(esc(apt.customer?.email || billTo?.email || ""), 42, y, 10);
  y -= 18;
  ...
  if (serviceCharge > 0) {
    drawSummary("Service charge", `Rs ${serviceCharge.toFixed(2)}`);
  }
