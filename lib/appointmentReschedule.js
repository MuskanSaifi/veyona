...
export async function sendRescheduleWhatsAppNotifications(appointment) {
  const userTemplate =
    process.env.INTERAKT_TEMPLATE_USER_RESCHEDULE || "transactional_user_appointment_rescheduled";
  const employeeTemplate =
    process.env.INTERAKT_TEMPLATE_EMPLOYEE_RESCHEDULE ||
  ...
  if (appointment.customer?.phone) {
    results.user = await sendWhatsAppTemplate(appointment.customer.phone, userTemplate, [
      servicesLabel,
      dateStr,
      timeStr,
    ]);
  ...
  if (appointment.employee?.phone) {
    ...
    results.employee = await sendWhatsAppTemplate(
      ...
      employeeTemplate,
      [clientName, servicesLabel, dateStr, timeStr]
    );
  }
