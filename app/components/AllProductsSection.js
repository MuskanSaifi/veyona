...
export default function AllProductsSection() {
  ...
  const [dentistCategories, setDentistCategories] = useState([]);
  ...
  useEffect(() => {
    if (salonCategories.length > 0 || dentistCategories.length > 0 || tattooCategories.length > 0) {
      fetchAllProducts();
    }
  }, [salonCategories, dentistCategories, tattooCategories]);
  ...
  const fetchAllCategories = async () => {
    try {
      ...
      const activeSalon = salonData.filter((c) => c.active);
      const activeDentist = dentistData.filter((c) => c.active);
      const activeTattoo = tattooData.filter((c) => c.active);
      
      setSalonCategories(activeSalon);
      setDentistCategories(activeDentist);
      setTattooCategories(activeTattoo);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const fetchAllProducts = async () => {
    try {
      ...
      const allCategories = [...salonCategories, ...dentistCategories, ...tattooCategories];
  ...
  return (
    <section className="w-full py-16 bg-gradient-to-b from-gray-50 to-white" id="products">
      <div className="w-full px-4 sm:px-6 lg:px-8 sm:px-6 lg:px-8">
        {/* Header */}
